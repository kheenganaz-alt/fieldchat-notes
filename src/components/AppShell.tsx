    ...
    <div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-slate-950 dark:text-white">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-slate-200/80 bg-white/80 px-4 py-5 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/75 lg:block">
        <div className="mb-8 flex items-center gap-3">
          <div className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-blue-600 to-violet-600 text-white shadow-soft">
        ...
        <button onClick={quickStart} className="mb-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-bold text-white shadow-soft dark:bg-white dark:text-slate-950">
          <MessageSquareText size={18} />
          Start Quick Session
        </button>
        <nav className="space-y-1">
          {items.map((item) => (
            <NavLink key={item.to} to={item.to} className={({ isActive }) => `flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold transition ${isActive ? "bg-blue-50 text-blue-700 dark:bg-blue-500/15 dark:text-blue-200" : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/5"}`}>
              <item.icon size={19} />
              {item.label}
            </NavLink>
          ))}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-slate-200 bg-white/92 px-2 pt-2 backdrop-blur-xl safe-bottom dark:border-white/10 dark:bg-slate-950/92 lg:hidden">
        {items.slice(0, 5).map((item) => (
          <NavLink key={item.to} to={item.to} className={({ isActive }) => `flex flex-col items-center gap-1 rounded-xl px-1 py-2 text-[11px] font-semibold ${isActive ? "text-blue-600 dark:text-blue-300" : "text-slate-500 dark:text-slate-400"}`}>
            <item.icon size={20} />
